...
# ---------------------------
# PaintBoardClient：Token 复用 + 自动重连
# ---------------------------
        ...
        while self.running:
            ...
            if not batch:
                ...
                if time.time() - last_log_time > 5:
                    ...
                    logger.info(
                        f"进度: {self.scheduler.done}/{self.scheduler.total} ({progress*100:.1f}%) | "
                        f"待修复: {len(self.scheduler.work_queue)} | "
                        f"活跃账户: {active}/{len(self.clients)}"
                    )
